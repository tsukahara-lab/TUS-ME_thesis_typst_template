...
#let thesis_init(body) = {
  ...
  show heading.where(level: 1): (
    it => {
      pagebreak()
      set text(font: gothic, size: 16pt)
      set par(first-line-indent: 0em)
      set align(center)

      if it.numbering != none {
        block(
          stroke: (bottom: (paint: black, dash: "dotted"), top: (paint: black, dash: "dotted")),
          fill: none,
          width: 100%,
        )[
          #v(2em)
  ...
  show heading.where(level: 2): (
    it => {
      set text(font: gothic, size: 14pt)
      block(above: 1.5em, below: 1.5em)[
        #context counter(heading).display()
        #h(1em)
        #it.body
  ...
  // 日本語間のコード改行を無効化・インライン数式の1/4スペースを追加
  show: cjk-spacer
...
#let thesis-appendix(body) = {
  ...
  show heading.where(level: 1): set heading(outlined: true, supplement: [付録])
    ...
    context {
      ...
      if alert-contents != () {
        ...
        for value in alert-contents {
          let set-fig-num = (
            str(counter(heading).at(value.at(0)).at(0))
              + "."
